...
description: Review a non-trivial refactor, subsystem, or cross-cutting architecture change in ELISEI. Use when evaluating proposals that affect unified content design, service and repository boundaries, API contracts, PostgreSQL schema, S3 and media storage, frontend and backend responsibilities, migration cost, extensibility, or operational risk.
---

# Review an Architecture Change for ELISEI

Используй этот skill, когда задача предлагает non-trivial refactor, новый subsystem или cross-cutting design change.

## Goal
Оценить, делает ли proposed change ELISEI более reliable, scalable и maintainable.
